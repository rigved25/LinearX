...
def evaluate_rnastralign_performance(
    data_path,
    pred_path,
    ct_path="./data/gold-database/",
    cnsns=False,
    get_msa_seq_info=False,
    skip_seq=True,
    allow_slip=True,
    verbose=False,
    backsearch=False,
):
    seq_files_name = sorted(
        [
            f
    ...
    if not skip_seq:
        assert len(seq_files_name) == len(struc_files_name), (
    ...
    for fid in range(len(seq_files_name)):
        if (
            len(seq_files_name) != len(struc_files_name)
            and seq_files_name[fid] + ".txt" not in struc_files_name
            and seq_files_name[fid] not in struc_files_name
        ):
        ...
        if get_msa_seq_info:
            ...
            seq_info[family].append(
                {"identity": msa_seq_identity, "avg_len": msa_seq_avg_len}
            )

        struc_lines = open(
            os.path.join(pred_path, struc_files_name[fid]), "r"
        ).readlines()
        if backsearch:
        ...
        assert len(ref_data) == len(pred_strucs), (
            "Number of reference structures and structures must be equal "
            + str(len(ref_data))
            + " "
            + str(len(pred_strucs))
        )
        ...
        for pred_struc, (ref_paired, ref_unpaired) in zip(pred_strucs, ref_data):
            ...
            assert len(pred_struc) == ref_len, (
                "Length of predicted structure and reference structure must be equal. "
                + str(len(pred_struc))
                + " "
                + str(ref_len)
            )
            precision, sensitivity, f1, structural_distance = utility.evaluate(
                pred_struc,
                gold_paired_pos_tuple=ref_paired,
                gold_unpaired_pos=ref_unpaired,
                allow_slip=allow_slip,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip-seq",
        action="store_true",
        help="skip sequence file if structure file is not found",
    )
    parser.add_argument("--verbose", action="store_true", help="print verbose output")
    parser.add_argument(
        "--cnsns",
        action="store_true",
        help="predicted structure is a single consensus structure",
    )
    parser.add_argument(
        "--allow-slip", action="store_true", help="allow slip in base pair matching"
    )
    parser.add_argument(
        "--seq-identity", action="store_true", help="calculate the sequence identity"
    )
    parser.add_argument(
        "--backsearch",
        action="store_true",
        help="search for the structure in reverse order",
    )
    ...
    (
        ...
        structural_distances,
        seq_info,
    ) = evaluate_rnastralign_performance(
        args.data_path,
        args.pred_path,
        args.ct_path,
        skip_seq=args.skip_seq,
        backsearch=args.backsearch,
        verbose=args.verbose,
        cnsns=args.cnsns,
        allow_slip=args.allow_slip,
        get_msa_seq_info=args.seq_identity,
    )
    ...
    header = "{:<8}\t{:>10}\t{:>10}\t{:>10}\t{:>20}".format(
        ...
        "Structural Distance (min, avg, max)",
    )
