...
int main(int argc, char* argv[]) {
    ...
    if (argc != 8) {
        std::cerr << "Usage: " << argv[0]
                  << " <msa_file_path> <energy_params> <num_iterations> <use_lazy_outside> <use_prev_outside_score> <restrict_search> "
                  << " <shrink_beam> \n";
        return EXIT_FAILURE;
    }
    ...
    bool restrict_search = std::stoi(argv[6]);
    int shrink_beam = std::stoi(argv[7]);
    ...
    try {
        ...
        LinearTurboFold ltf(&mseq, energy_params, num_itr, use_lazy_outside, use_prev_outside_score, restrict_search, shrink_beam, 0.3,
