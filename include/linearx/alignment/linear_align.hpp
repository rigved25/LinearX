...
class LinearAlignment {
   ...
   private:
    ...
    std::vector<std::pair<double, std::pair<int, int>>>  beam_scores;
    ...
    void update_state_alpha(const Mode mode, HState &state, const double new_score);
    ...
    std::pair<unsigned long, unsigned long> backward_update(const int i, const int j, const HState &state,
                                                            const HStateType type, const double edge_threshold);
   ...
   protected:
    virtual unsigned beam_prune(std::unordered_map<std::pair<int, int>, HState, linearx::utils::PairHash> &beamstep, const int beam_size);
   ...
   public:
    ...
    std::vector<std::unordered_map<int, double>> coinc_prob;
    std::vector<std::vector<int>> prob_rev_idx;

    double alpha1, alpha2, alpha3;

    LinearAlignment(Sequence &seq1, Sequence &seq2, bool verbose = false, double alpha1 = 1.0, double alpha2 = 0.8,
                    double alpha3 = 0.5);

    void use_prob_set1();
    void use_prob_set2(const float similarity);

    void reset_beams(const bool freeMemory = true);
    ...
    double get_trans_emit_prob(const int i, const int j, const HStateType h, const HStateType h_prev) const;
    double get_match_score(const int i, const int j) const;
